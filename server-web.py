...
ENABLE_CALLS = os.getenv("ENABLE_CALLS", "0") == "1"

# --- AIORTC (если звонки включены) ---
if ENABLE_CALLS:
    try:
        from aiortc import RTCPeerConnection, RTCSessionDescription
        from aiortc.contrib.media import MediaRelay
        logging.info("✅ aiortc импортирован")
    except ImportError:
        logging.error("❌ aiortc не установлен. Звонки не будут работать.")
        ENABLE_CALLS = False
...
def activate_premium(user_id: int, plan: str) -> bool:
    if plan == '1day':
        info = premium_users.get(user_id, {})
        if info.get('trial_used', False):
            logger.warning(f"⚠️ Попытка повторного использования пробного периода пользователем {user_id}")
            return False
        info['trial_used'] = True
        info['plan'] = '1day'
        info['expiry'] = (datetime.now() + timedelta(days=1)).isoformat()
        info['activated_at'] = datetime.now().isoformat()
        premium_users[user_id] = info
    else:
        expiry = None
        if plan == '1week':
            expiry = (datetime.now() + timedelta(weeks=1)).isoformat()
        elif plan == '1month':
        ...
        premium_users[user_id] = {
            ...
            'trial_used': premium_users.get(user_id, {}).get('trial_used', False)
        }
...
def add_channel_subscriber(channel_id: str, user_id: int):
    ...
    cursor.execute("INSERT OR IGNORE INTO chat_members (chat_id, user_id, role) VALUES (?, ?, 'subscriber')", (channel_id, user_id))
    conn.commit()
    conn.close()
...
def get_chat_info(chat_id: str, user_id: int) -> Optional[Dict]:
    ...
    if result['type'] == 'channel':
        ...
        cursor.execute("SELECT 1 FROM channel_subscribers WHERE channel_id = ? AND user_id = ? AND unsubscribed_at IS NULL", (chat_id, user_id))
        result['is_subscribed'] = cursor.fetchone() is not None
    conn.close()
    return result
...
def save_user_session(session_id: str, user_id: int, device: str, browser: str):
    conn = sqlite3.connect(DATABASE)
    cursor = conn.cursor()
    cursor.execute("INSERT OR REPLACE INTO user_sessions (id, user_id, device, browser, last_active) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)", (session_id, user_id, device, browser))
    conn.commit()
    conn.close()
...
async def auto_ping():
    PING_URL = "https://server-web-crow.onrender.com"
    while True:
...
call_connections: Dict[str, RTCPeerConnection] = {}
call_relay = MediaRelay() if ENABLE_CALLS else None

async def handle_voice_call(websocket, user_id, peer_id):
    """Управление WebRTC-звонком через сервер."""
    if not ENABLE_CALLS:
        return
    pc = RTCPeerConnection()
    call_id = f"call_{user_id}_{peer_id}"
    call_connections[call_id] = pc

    @pc.on("track")
    async def on_track(track):
        logger.info(f"🎧 Трек от {user_id}: {track.kind}")
        if peer_id in active_connections:
            peer_pc = call_connections.get(f"call_{peer_id}_{user_id}")
            if peer_pc:
                relay_track = call_relay.subscribe(track)
                peer_pc.addTrack(relay_track)

    @pc.on("connectionstatechange")
    async def on_connectionstatechange():
        state = pc.connectionState
        logger.info(f"🔗 Состояние звонка {user_id}: {state}")
        if state == "failed":
            if user_id in active_connections:
...
async def ws_handler(websocket):
    ...
    if is_banned(client_ip):
        logger.warning(f"🚫 Заблокирован запрос от забаненного IP")
    ...
    try:
        async for message in websocket:
            if not check_rate_limit(client_ip):
                logger.warning(f"🚫 IP превысил лимит запросов")
            ...
            try:
                ...
                requires_auth = command in [
                    ...
                    "SUBSCRIBE_CHANNEL", "UNSUBSCRIBE_CHANNEL", "JOIN_GROUP",
                    "SEARCH_CHANNELS"
                ]
                ...
                # --- ВХОД ---
                    ...
                    save_user_session(session_id, user_id, device, browser)
                ...
                # --- АКТИВАЦИЯ PREMIUM ---
                    ...
                    plan = data.get("plan")
                    if not plan:
                        await websocket.send(json.dumps({"error": "План не указан"}))
                        continue
                    if plan == "1day":
                        if not activate_premium(user_id, plan):
                    ...
                    if not chat_id or not text:
                        await websocket.send(json.dumps({"error": "Недостаточно данных"}))
                        continue
                    if user_id is None or not is_chat_member(chat_id, user_id):
                    ...
                    chat_type_row = cursor.fetchone()
                    if chat_type_row and chat_type_row[0] == 'channel':
                        cursor.execute("SELECT role FROM chat_members WHERE chat_id = ? AND user_id = ? AND left_at IS NULL", (chat_id, user_id))
                        role_row = cursor.fetchone()
                        conn.close()
                        if role_row and role_row[0] not in ('owner', 'admin'):
                            await websocket.send(json.dumps({"error": "Только владелец канала может писать"}))
                            continue
                    else:
                        conn.close()
                    ...
                    sender_nick = sender['nickname'] if sender else f"User {user_id}"
                    ...
                    if chat_type_row and chat_type_row[0] == 'channel':
                        ...
                        conn.close()
                        if role_row and role_row[0] not in ('owner', 'admin'):
                            await websocket.send(json.dumps({"error": "Только владелец канала может отправлять стикеры"}))
                            continue
                    else:
                        conn.close()
                    ...
                    if chat_type_row and chat_type_row[0] == 'channel':
                        ...
                        conn.close()
                        if role_row and role_row[0] not in ('owner', 'admin'):
                            await websocket.send(json.dumps({"error": "Только владелец канала может отправлять голосовые"}))
                            continue
                    else:
                        conn.close()
                    ...
                    if chat_type_row and chat_type_row[0] == 'channel':
                        ...
                        conn.close()
                        if role_row and role_row[0] not in ('owner', 'admin'):
                            await websocket.send(json.dumps({"error": "Только владелец канала может отправлять файлы"}))
                            continue
                    else:
                        conn.close()
                ...
                # --- ЗВОНКИ (С ОБРАБОТКОЙ ICE_CANDIDATE) ---
                    ...
                    if target_user_id in active_connections:
                        await active_connections[target_user_id].send(json.dumps({
                            "cmd": "CALL_REQUEST",
                            "from_id": user_id,
                            "from_name": data.get("caller_name"),
                    ...
                    target_user_id = int(data.get("target_user_id"))
                    if target_user_id in active_connections and ENABLE_CALLS:
                        asyncio.create_task(handle_voice_call(websocket, user_id, target_user_id))
                        asyncio.create_task(handle_voice_call(active_connections[target_user_id], target_user_id, user_id))
                        await active_connections[target_user_id].send(json.dumps({
                            "cmd": "CALL_ACCEPTED",
                            "from_id": user_id
                        }))
                        ...
                        await websocket.send(json.dumps({"error": "Пользователь не в сети или звонки отключены"}))
                    ...
                    call_id_1 = f"call_{user_id}_{target_user_id}"
                    call_id_2 = f"call_{target_user_id}_{user_id}"
                    for cid in (call_id_1, call_id_2):
                        if cid in call_connections:
                            await call_connections[cid].close()
                    await websocket.send(json.dumps({"status": "CALL_ENDED"}))

                # ========== ДОБАВЛЕННЫЙ ОБРАБОТЧИК ICE_CANDIDATE ==========
                elif command == "ICE_CANDIDATE":
                    target_user_id = data.get("target_user_id")
                    candidate = data.get("candidate")
                    if not target_user_id or not candidate:
                        await websocket.send(json.dumps({"error": "Не хватает данных для ICE_CANDIDATE"}))
                        continue
                    if target_user_id in active_connections:
                        try:
                            await active_connections[target_user_id].send(json.dumps({
                            ...
                            logger.info(f"🔄 Передан ICE-кандидат от {user_id} к {target_user_id}")
                        except Exception as e:
                            logger.error(f"❌ Ошибка при передаче ICE-кандидата: {e}")
                    else:
                        logger.warning(f"⚠️ Пользователь {target_user_id} не в сети")

                # --- ОСТАЛЬНЫЕ КОМАНДЫ (поиск, профиль, группы, каналы и т.д.) ---
                    ...
                    for user in users:
                        uid = user["id"]
                        user["is_online"] = user["is_online"] and uid not in hidden_online_users
                        user["last_seen_display"] = format_last_seen(user["last_seen"], uid in hidden_last_seen_users)
                    await websocket.send(json.dumps({
                        "cmd": "SEARCH_RESULTS",
                        "users": users
                    }))
                    ...
                    if not target_id or user_id is None:
                        await websocket.send(json.dumps({"error": "Недостаточно данных"}))
                        continue
                ...
                elif command == "SUBSCRIBE_CHANNEL":
                    ...
                    conn = sqlite3.connect(DATABASE)
                    cursor = conn.cursor()
                    cursor.execute("SELECT 1 FROM channel_subscribers WHERE channel_id = ? AND user_id = ? AND unsubscribed_at IS NULL", (channel_id, user_id))
                    if cursor.fetchone():
                        conn.close()
                        await websocket.send(json.dumps({"error": "Уже подписаны"}))
                        continue
                    conn.close()
                    add_channel_subscriber(channel_id, user_id)
                    await websocket.send(json.dumps({"status": "SUBSCRIBED"}))

                elif command == "UNSUBSCRIBE_CHANNEL":
                    channel_id = data.get("channel_id")
                    if not channel_id or user_id is None:
                        await websocket.send(json.dumps({"error": "Недостаточно данных"}))
                        continue
                    ...
                    conn = sqlite3.connect(DATABASE)
                    cursor = conn.cursor()
                    cursor.execute("SELECT type FROM chats WHERE id = ? AND type = 'group'", (group_id,))
                    if not cursor.fetchone():
                        conn.close()
                        await websocket.send(json.dumps({"error": "Группа не найдена"}))
                        continue
                    conn.close()
                    add_chat_member(group_id, user_id, 'member')
                    await websocket.send(json.dumps({"status": "JOINED_GROUP"}))
                    ...
                    cursor.execute('''
                        SELECT c.id, c.name, c.type,
                               (SELECT COUNT(*) FROM chat_members WHERE chat_id = c.id AND left_at IS NULL) as members_count,
                               (SELECT COUNT(*) FROM channel_subscribers WHERE channel_id = c.id AND unsubscribed_at IS NULL) as subscribers_count
                        FROM chats c
                        WHERE (c.type = 'channel' OR c.type = 'group') AND c.name LIKE ? AND c.deleted_at IS NULL
                        LIMIT 20
                    ''', (f'%{query}%',))
                    ...
                    target_user_id = data.get("target_user_id")
                    if not target_user_id or user_id is None:
                        await websocket.send(json.dumps({"error": "Недостаточно данных"}))
                        continue
                    ...
                    if not from_user_id or user_id is None:
                        await websocket.send(json.dumps({"error": "Недостаточно данных"}))
                        continue
                    ...
                    if not nickname or user_id is None:
                        await websocket.send(json.dumps({"error": "Недостаточно данных"}))
                        continue
                    conn = sqlite3.connect(DATABASE)
                    cursor = conn.cursor()
                    ...
                    if not session_to_remove or user_id is None:
                        await websocket.send(json.dumps({"error": "Недостаточно данных"}))
                        continue
