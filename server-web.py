    ...
    try:
        async for message in websocket:
            ...
            try:
                ...
                # --- ПОЛУЧЕНИЕ СООБЩЕНИЙ (АНОНИМНО - НЕ ХРАНИМ) ---
                ...
                # --- ОТПРАВКА ТЕКСТОВОГО СООБЩЕНИЯ (ТОЛЬКО ПЕРЕСЫЛКА) ---
                    ...
                    sender_has_premium = is_premium(user_id)
                ...
                # --- ОТПРАВКА СТИКЕРА (ТОЛЬКО ПЕРЕСЫЛКА) ---
                ...
                # --- ОТПРАВКА ГОЛОСОВОГО (ТОЛЬКО ПЕРЕСЫЛКА) ---
                ...
                # --- УСТАНОВКА РЕАКЦИИ (ТОЛЬКО ПЕРЕСЫЛКА) ---
                elif command == "SET_REACTION":
                    chat_id = data.get("chat_id")
                    ...
                    user = get_user_by_id(user_id)
                    from_name = user.get("nickname", f"User {user_id}") if user else f"User {user_id}"
                    ...
                    for member_id in members:
                        if member_id != user_id and member_id in active_connections:
                            try:
                                ...
                                pass
                    await websocket.send(json.dumps({"status": "REACTION_SET"}))
                ...
                # --- ОТПРАВКА ФАЙЛА (НЕ СОХРАНЯЕТСЯ, ТОЛЬКО ПЕРЕСЫЛКА) ---
...
async def main():
    try:
        ...
        async with websockets.serve(
            ...
            ping_interval=20,
            ping_timeout=60,
            max_size=10 * 1024 * 1024  # 10 МБ для файлов/голосовых
        ) as server:
            ...
            logger.info(f"📎 Поддержка файлов: включена (НЕ сохраняются)")
            logger.info(f"🎨 Поддержка стикеров: включена")
            logger.info(f"🎤 Поддержка голосовых: включена")
            logger.info(f"❤️ Поддержка реакций: включена")
            logger.info(f"📞 Поддержка звонков: включена")
            logger.info(f"🔐 Полная анонимность: сообщения НЕ хранятся")
