...
# --- АНОНИМНАЯ ЗАЩИТА ОТ МУЛЬТИАККАУНТОВ (через anon_id, без IP) ---
MAX_ACCOUNTS_PER_ANON_ID = 2
anon_registration_count = defaultdict(int)   # anon_id -> количество аккаунтов
...
def check_rate_limit(ip: str) -> bool:
    ...
    if len(request_counts[ip]) >= RATE_LIMIT['max_requests']:
        ...
        logger.warning(f"🚫 IP забанен на {RATE_LIMIT['ban_time']}с")
        return False
...
def create_user(login: str, nickname: str, password: str) -> int:
    ...
    conn = sqlite3.connect(DATABASE)
    cursor = conn.cursor()
    cursor.execute("INSERT INTO users (login, nickname, password_salt, password_hash, last_seen, is_online) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP, 1)", (login, nickname, salt, hash_hex))
    ...
    try:
        async for message in websocket:
            ...
            try:
                ...
                # --- РЕГИСТРАЦИЯ (с проверкой anon_id) ---
                if command == "REGISTER":
                    ...
                    if len(password) < 6:
                        ...
                        continue
                    if not anon_id:
                        await websocket.send(json.dumps({"error": "Отсутствует анонимный идентификатор"}))
                        continue

                    if anon_registration_count.get(anon_id, 0) >= MAX_ACCOUNTS_PER_ANON_ID:
                        await websocket.send(json.dumps({"error": "Достигнут лимит аккаунтов для этого устройства"}))
                        continue
                    ...
                    anon_registration_count[anon_id] = anon_registration_count.get(anon_id, 0) + 1
                    ...
                    logger.info(f"✅ Пользователь зарегистрирован: ID {user_id} (anon_id: {anon_id})")
                    continue
                ...
                if command == "GET_CHATS":
                    ...
                    try:
                        ...
                        logger.info(f"📋 Отправлен список чатов для {user_id}: {len(chats)} чатов")
                    except Exception as e:
                        logger.error(f"❌ Ошибка GET_CHATS: {e}")
                        await websocket.send(json.dumps({"error": "Ошибка получения чатов"}))
                    continue
                ...
                # --- УДАЛЕНИЕ АККАУНТА (с освобождением слота) ---
                if command == "DELETE_ACCOUNT":
                    if user_id is None:
                        await websocket.send(json.dumps({"error": "Не авторизован"}))
                        continue
                    anon_id = data.get("anon_id")
                    if anon_id and anon_registration_count.get(anon_id, 0) > 0:
                        anon_registration_count[anon_id] -= 1
                    if delete_user_account(user_id):
                        await websocket.send(json.dumps({"status": "ACCOUNT_DELETED"}))
                    else:
                        await websocket.send(json.dumps({"error": "Ошибка удаления"}))
                    await websocket.close()
                    continue
