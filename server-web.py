...
# --- АНОНИМНАЯ ЗАЩИТА ОТ МУЛЬТИАККАУНТОВ (постоянный лимит 2 на IP, без сохранения) ---
MAX_ACCOUNTS_PER_IP = 2
ip_registration_count = defaultdict(int)  # IP -> сколько аккаунтов создано (только в памяти)
...
def can_register_from_ip(ip: str) -> bool:
    """Проверка постоянного лимита аккаунтов с одного IP (без временного окна)."""
    return ip_registration_count[ip] < MAX_ACCOUNTS_PER_IP
    ...
    try:
        async for message in websocket:
            ...
            try:
                ...
                # --- РЕГИСТРАЦИЯ (с анонимным постоянным лимитом) ---
                if command == "REGISTER":
                    ...
                    # Проверка лимита аккаунтов с этого IP (постоянный, без временного окна)
                    ...
                    # Увеличиваем счётчик регистраций с этого IP
                    ip_registration_count[client_ip] += 1
