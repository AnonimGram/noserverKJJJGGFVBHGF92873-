        ...
        async for message in websocket:
            ...
            try:
                ...
                # --- ЗВОНОК: ЗАПРОС ---
                    ...
                    if target_user_id in active_connections:
                        try:
                            ...
                            logger.info(f"📞 Звонок: {user_id} -> {target_user_id}")
                        except Exception as e:
                            logger.error(f"Ошибка отправки CALL_REQUEST: {e}")
                ...
                # --- ЗВОНОК: ПРИНЯТИЕ ---
                    ...
                    if target_user_id in active_connections:
                        try:
                            ...
                            logger.info(f"✅ Звонок принят: {user_id} <- {target_user_id}")
                        except Exception as e:
                            logger.error(f"Ошибка отправки CALL_ACCEPT: {e}")
                    else:
                        await websocket.send(json.dumps({"error": "Пользователь не в сети"}))
                ...
                # --- ЗВОНОК: ОТКЛОНЕНИЕ ---
                    ...
                    if target_user_id in active_connections:
                        try:
                            ...
                            logger.error(f"Ошибка отправки CALL_REJECT: {e}")
                    else:
                        await websocket.send(json.dumps({"error": "Пользователь не в сети"}))
                ...
                # --- ICE CANDIDATE ---
                    ...
                    if not target_user_id or not candidate or user_id is None:
                        continue
                    target_user_id = int(target_user_id)
                    if target_user_id in active_connections:
                        try:
