...
async def auto_ping():
    ...
    while True:
        ...
        try:
            import aiohttp
            # Используем публичный URL для пинга
    ...
    try:
        async for message in websocket:
            ...
            try:
                ...
                # --- ОТПРАВКА ФАЙЛА (с немедленным удалением) ---
                elif command == "SEND_FILE":
                    chat_id = data.get("chat_id")
                    file_name = data.get("fileName")
                    file_size = data.get("fileSize")
                    file_data = data.get("fileData")  # base64 строка
                    ...
                    # ВРЕМЕННО СОЗДАЁМ ФАЙЛ (только для пересылки, сразу удалим)
                    temp_file = None
                    try:
                        # Декодируем base64
                        if ',' in file_data:
                        ...
                        # Читаем для пересылки (можно передавать как base64)
                        file_base64 = base64.b64encode(file_bytes).decode('utf-8')
                        
                        # Пересылаем всем участникам
                        for member_id in members:
                            if member_id in active_connections:
                                try:
                                    await active_connections[member_id].send(json.dumps({
