...
# --- БАЗА ДАННЫХ (с защитой от мультиаккаунтов) ---
def init_database():
    ...
    # Таблицы для защиты от мультиаккаунтов
    cursor.execute('''
    ...
    CREATE TABLE IF NOT EXISTS device_accounts (
        ...
        FOREIGN KEY (token) REFERENCES device_tokens(token) ON DELETE CASCADE,
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
    )
    ''')
    conn.commit()
    ...
    try:
        async for message in websocket:
            ...
            try:
                ...
                # --- ПОЛУЧЕНИЕ DEVICE TOKEN (для защиты от мультиаккаунтов) ---
                if command == "GET_DEVICE_TOKEN":
                    token = secrets.token_urlsafe(32)
                    conn = sqlite3.connect(DATABASE)
                    cursor = conn.cursor()
                    cursor.execute("INSERT INTO device_tokens (token) VALUES (?)", (token,))
                    conn.commit()
                    conn.close()
                    await websocket.send(json.dumps({"cmd": "DEVICE_TOKEN", "token": token}))
                    continue

                # --- РЕГИСТРАЦИЯ (с проверкой device_token) ---
                if command == "REGISTER":
                    ...
                    if len(password) < 6:
                        ...
                        continue
                    if not device_token:
                        await websocket.send(json.dumps({"error": "Отсутствует идентификатор устройства"}))
                        continue

                    conn = sqlite3.connect(DATABASE)
                    cursor = conn.cursor()
                    cursor.execute("SELECT 1 FROM device_tokens WHERE token = ?", (device_token,))
                    if not cursor.fetchone():
                        conn.close()
                        await websocket.send(json.dumps({"error": "Недействительный идентификатор устройства. Перезапустите приложение."}))
                        continue
                    cursor.execute("SELECT COUNT(*) FROM device_accounts WHERE token = ?", (device_token,))
                    count = cursor.fetchone()[0]
                    LIMIT = 2  # максимум 2 аккаунта на одно устройство
                    if count >= LIMIT:
                        conn.close()
                        await websocket.send(json.dumps({"error": f"Превышен лимит аккаунтов на этом устройстве (максимум {LIMIT})"}))
                        continue

                    existing = get_user_by_login(login)
                    if existing:
                        conn.close()
                        await websocket.send(json.dumps({"error": "Пользователь уже существует"}))
                        continue

                    user_id = create_user(login, login, password)
                    cursor.execute("INSERT INTO device_accounts (token, user_id) VALUES (?, ?)", (device_token, user_id))
                    conn.commit()
                    conn.close()
                    ...
                    logger.info(f"✅ Пользователь зарегистрирован: ID {user_id} (device_token: {device_token[:8]}...)")
                    continue
                ...
                if command == "GET_CHATS":
                    ...
                    for chat in chats:
                        if chat["type"] == "private" and chat["id"].startswith("private_"):
                ...
                # --- ICE CANDIDATE (обязательно для звонков) ---
                ...
                # --- ОТПИСКА ОТ КАНАЛА ---
                if command == "UNSUBSCRIBE_CHANNEL":
                    channel_id = data.get("channel_id")
                    if not channel_id or user_id is None:
                        await websocket.send(json.dumps({"error": "Недостаточно данных"}))
                        continue
                ...
                if command == "GET_FRIEND_REQUESTS":
                    if user_id is None:
                        await websocket.send(json.dumps({"error": "Не авторизован"}))
                        continue
                ...
                if command == "REJECT_FRIEND_REQUEST":
                    from_user_id = data.get("from_user_id")
                    if not from_user_id or user_id is None:
                        await websocket.send(json.dumps({"error": "Недостаточно данных"}))
                        continue
